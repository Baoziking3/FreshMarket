...
class PreferenceUtil{
  ...
  async loadPreference(context){
    try { // 加载preferences
      this.pref = await preferences.getPreferences(context, CommonConstants.F_MARKET)
      Logger.debug(`加载Preferences[${CommonConstants.F_MARKET}]成功`)
    } catch (e) {
      Logger.debug(`加载Preferences[${CommonConstants.F_MARKET}]失败`, JSON.stringify(e))
    }
  }

  async putPreferenceValue(key: string, value: preferences.ValueType){
    if (!this.pref) {
      Logger.debug(`Preferences[${CommonConstants.F_MARKET}]尚未初始化！`)
      return
    }
    try {
      // 写入数据
